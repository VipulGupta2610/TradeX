...
export default function Leaderboard() {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] dark:bg-[#020202] text-black dark:text-white transition-colors duration-500 font-sans selection:bg-amber-500/30 relative overflow-x-hidden">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden fixed bg-[#020202]">
        ...
        <div className="absolute inset-0 opacity-[0.05] dark:opacity-[0.08]" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='103.923' viewBox='0 0 60 103.923' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M30 0l25.98 15v30L30 60 4.02 45V15zM30 103.923l25.98-15v-30l-25.98-15-25.98 15v30z' fill='none' stroke='white' stroke-width='1'/%3E%3C/svg%3E")`, backgroundSize: '60px 103.923px' }} />
      ...
      <nav className="relative z-50 px-8 py-6 flex justify-between items-center border-b border-black/5 dark:border-white/5 backdrop-blur-xl bg-white/40 dark:bg-[#020202]/40">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold tracking-[0.2em] text-sm text-zinc-800 dark:text-zinc-300">HALL OF FAME</span>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <a href="/Dashboard" className="px-6 py-2.5 rounded-full bg-amber-500 text-black font-bold text-sm hover:scale-[1.02] transition-transform shadow-[0_0_20px_rgba(245,158,11,0.2)]">
            Execution Terminal
          </a>
        </div>
      </nav>
      ...
      <main className="relative z-10 w-full max-w-[1400px] mx-auto px-6 md:px-12 py-12">
        <motion.div variants={container} initial="hidden" animate="show" className="space-y-12">
          ...
          <motion.div variants={item} className="flex flex-col md:flex-row justify-center items-end gap-6 pt-12 pb-8">
            {podium.map((trader, i) => {
              // i === 0 is Rank 2 (Silver), i === 1 is Rank 1 (Gold), i === 2 is Rank 3 (Bronze)
              const isFirst = trader.rank === 1;
              const isSecond = trader.rank === 2;
              const isThird = trader.rank === 3;
              
              const borderGlow = isFirst ? 'border-amber-400/50 shadow-[0_0_40px_rgba(251,191,36,0.15)]' : isSecond ? 'border-zinc-300/50 shadow-[0_0_30px_rgba(212,212,216,0.1)]' : 'border-orange-500/50 shadow-[0_0_30px_rgba(249,115,22,0.1)]';
              const rankColor = isFirst ? 'bg-amber-400 text-black' : isSecond ? 'bg-zinc-300 text-black' : 'bg-orange-500 text-black';
              ...
              return (
                <div key={trader.rank} className={`relative flex flex-col items-center w-full md:w-[30%] ${isFirst ? 'md:-translate-y-8 z-20' : 'z-10'}`}>
                  ...
                  <div className={`w-full bg-white/60 dark:bg-[#0A0A0A]/80 backdrop-blur-3xl border ${borderGlow} rounded-[32px] p-8 flex flex-col items-center text-center relative overflow-hidden group hover:-translate-y-2 transition-transform duration-300`}>
                    ...
                    <div className={`w-20 h-20 rounded-full flex items-center justify-center font-black text-2xl mb-4 border-2 ${isFirst ? 'border-amber-400 bg-amber-400/10 text-amber-400' : isSecond ? 'border-zinc-300 bg-zinc-300/10 text-zinc-300' : 'border-orange-500 bg-orange-500/10 text-orange-500'}`}>
                      {trader.avatar}
                    </div>
                    ...
                    <div className="w-full space-y-3">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-mono font-bold text-white">{trader.roi}</span>
                      </div>
                      <div className="flex justify-between items-center">
                    ...
                    {restOfTraders.map((trader, index) => (
                      ...
                      >
                        ...
                        <td className="py-5 pl-4 rounded-l-2xl">
                          <span className="font-mono font-bold text-zinc-500 group-hover:text-white transition-colors text-base">#{trader.rank}</span>
                        </td>
                        ...
                        <td className="py-5 text-center">
                          <span className={`inline-block px-3 py-1 rounded-md text-[10px] font-bold uppercase tracking-widest border ${
                            trader.tier === 'Diamond' ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20' : 
                            trader.tier === 'Platinum' ? 'bg-purple-500/10 text-purple-400 border-purple-500/20' :
