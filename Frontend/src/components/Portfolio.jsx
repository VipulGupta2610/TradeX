...
export default function PortfolioUltra() {
  ...
  const displayHoldings = positions && positions.length > 0 
  ...
  const displayMetrics = metrics && balance !== undefined
    ? {
        totalBalance: `₹${(metrics.totalEquity || 0).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`,
        cash: `₹${(balance || 0).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`,
        dailyPnL: `${metrics.unrealizedPnl >= 0 ? '+' : '-'}₹${Math.abs(metrics.unrealizedPnl || 0).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`,
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4 text-green-500 font-semibold">
                      ...
                      <span className="px-3 py-1 rounded-full bg-green-100 text-green-700 text-sm font-bold flex items-center gap-1 border border-green-200">
                        ...
                        {displayMetrics.totalPnLPercent}
                      </span>
                    </div>
                  </div>
              ...
              <motion.div variants={item} className="xl:col-span-4 flex flex-col gap-8">
                <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-lg hover:shadow-2xl transition-shadow duration-300">
                  ...
                  <h3 className="text-4xl font-extrabold">{displayMetrics.cash}</h3>
                  ...
                  <div className="space-y-6">
                    <div>
                      <div className="flex justify-between mb-2 text-sm font-semibold">
                        <span className="flex items-center gap-2"><span className="w-3 h-3 bg-green-400 rounded-full shadow-inner" /> Cryptocurrencies</span>
                    ...
                    <div>
                      <div className="flex justify-between mb-2 text-sm font-semibold">
                        <span className="flex items-center gap-2"><span className="w-3 h-3 bg-blue-500 rounded-full shadow-inner" /> Global Equities</span>
                    ...
                    <div>
                      <div className="flex justify-between mb-2 text-sm font-semibold">
                        <span className="flex items-center gap-2"><span className="w-3 h-3 bg-purple-400 rounded-full shadow-inner" /> Cash & Forex</span>
                ...
                <table className="w-full min-w-[1000px] border-collapse text-sm">
                  <thead>
                    <tr className="uppercase text-gray-400 border-b border-gray-200 dark:border-gray-700 text-[10px] font-semibold tracking-widest">
                      <th className="py-4 pl-4 text-left">Asset</th>
                  ...
                  <tbody>
                    <AnimatePresence>
                      {displayHoldings.map((asset, index) => (
                        <motion.tr
                          key={asset.id}
