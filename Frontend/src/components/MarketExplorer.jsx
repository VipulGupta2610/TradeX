...
export default function MarketExplorer() {
  ...
  return (
    ...
    <div className="flex h-screen bg-[#FAFAFA] dark:bg-[#020202] text-black dark:text-white transition-colors duration-500 font-sans selection:bg-purple-500/30 overflow-hidden">
      ...
      <div className="flex-1 relative overflow-y-auto overflow-x-hidden">
        ...
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden bg-[#FAFAFA] dark:bg-[#020202] transition-colors duration-500">
          ...
          <div className="absolute inset-0 opacity-[0.03] dark:opacity-[0.1]" style={{ 
            backgroundImage: `url("data:image/svg+xml,%3Csvg width='400' height='400' viewBox='0 0 400 400' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M 100 200 Q 150 100 200 200 T 300 200' fill='none' stroke='currentColor' stroke-width='1'/%3E%3Cpath d='M 50 250 Q 150 150 250 250 T 350 250' fill='none' stroke='currentColor' stroke-width='0.5'/%3E%3Cpath d='M 150 300 Q 200 200 300 300 T 400 300' fill='none' stroke='currentColor' stroke-width='0.5'/%3E%3C/svg%3E")`, 
                ...
                {macroMetrics.map((metric, i) => (
                  <div key={i} className="bg-white/60 dark:bg-[#0A0A0A]/80 backdrop-blur-xl border border-black/5 dark:border-white/[0.08] rounded-2xl p-5 min-w-[180px] shadow-sm relative overflow-hidden group">
                    ...
                    <div className="flex items-end gap-2">
                      <p className={`text-2xl font-mono font-black ${metric.color || 'text-black dark:text-white'}`}>{metric.value}</p>
                      {metric.trend && (
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
              ...
              <motion.div variants={item} className="xl:col-span-8 flex flex-col gap-6">
                ...
                <div className="bg-white/60 dark:bg-[#0A0A0A]/80 backdrop-blur-3xl border border-black/5 dark:border-white/[0.08] rounded-[40px] shadow-2xl overflow-hidden flex flex-col flex-1 min-h-[500px]">
                  <div className="px-8 py-6 border-b border-black/5 dark:border-white/[0.05] flex justify-between items-center bg-black/[0.02]">
                    ...
                    <div className="flex p-1.5 bg-black/5 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/[0.05]">
                      {['S&P 500', 'Crypto', 'Forex'].map(tab => (
                        <button 
                          ...
                          className={`px-4 py-1.5 rounded-xl text-xs font-bold transition-all duration-300 ${activeTab === tab ? 'bg-white dark:bg-[#1A1A1A] text-black dark:text-white shadow-sm' : 'text-zinc-500 hover:text-black dark:hover:text-white'}`}
                        >
                          {tab}
                        </button>
                      ))}
              ...
              <motion.div variants={item} className="xl:col-span-4 flex flex-col gap-6">
                    ...
                    {sectorFlows.map((sector, idx) => (
                      <div key={idx} className="group/bar">
                        <div className="flex justify-between items-end mb-2">
                          <span className="text-sm font-bold text-zinc-600 dark:text-zinc-300">{sector.name}</span>
                          <div className="text-right">
                            <p className="font-mono text-xs font-bold text-black dark:text-white">{sector.flow}</p>
                          </div>
                        </div>
                        <div className="w-full h-2 bg-black/10 dark:bg-[#111] rounded-full overflow-hidden border border-black/5 dark:border-white/5 relative">
                          {/* Center Zero Line for Visual Reference */}
                          <div className="absolute top-0 bottom-0 left-[20%] w-px bg-black/20 dark:bg-white/20 z-10" />
                          <motion.div 
                            initial={{ width: 0 }} 
                ...
                <div className="bg-white/60 dark:bg-[#0A0A0A]/80 backdrop-blur-3xl border border-black/5 dark:border-white/[0.08] rounded-[40px] p-8 shadow-xl flex-1 flex flex-col">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <span className="flex items-center gap-1.5 px-2 py-1 rounded bg-purple-500/10 text-purple-600 dark:text-purple-400 text-[10px] font-bold border border-purple-500/20">
                      <span className="w-1.5 h-1.5 rounded-full bg-purple-500 dark:bg-purple-400 animate-pulse" /> Live
                    </span>
                  </div>
                  ...
                  <div className="space-y-3 flex-1 overflow-y-auto scrollbar-hide">
                    {[
                      ...
                      <div key={i} className="flex justify-between items-center p-3 rounded-2xl bg-black/5 dark:bg-[#050505] border border-black/5 dark:border-white/5 hover:border-black/10 dark:hover:border-white/10 transition-colors cursor-pointer group/card">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-black/5 dark:bg-white/5 flex items-center justify-center font-bold text-xs">
                            {asset.ticker[0]}
                          </div>
                          <div>
                            <p className="font-bold text-sm group-hover/card:text-purple-500 dark:group-hover/card:text-purple-400 transition-colors">{asset.ticker}</p>
                  ...
                  <Link to="/Markets" className="block text-center w-full mt-6 py-3 rounded-xl border border-black/10 dark:border-white/10 text-sm font-bold hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                    View Full Screener
                  </Link>
                </div>
