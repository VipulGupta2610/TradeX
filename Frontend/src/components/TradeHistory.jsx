    ...
    <div className="flex flex-col md:flex-row h-[100dvh] bg-[#FAFAFA] dark:bg-[#020202] text-black dark:text-white transition-colors duration-500 font-sans selection:bg-emerald-500/30 overflow-hidden">
      ...
      <div className="flex-1 w-full relative overflow-y-auto overflow-x-hidden">
        ...
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
          <div className="absolute top-[-10%] right-[-5%] w-[80vw] h-[80vw] md:w-[50vw] md:h-[50vw] max-w-[800px] max-h-[800px] bg-[radial-gradient(ellipse_at_center,rgba(16,185,129,0.06),transparent_60%)] blur-[100px]" />
          <div className="absolute bottom-[-10%] left-[-5%] w-[60vw] h-[60vw] md:w-[40vw] md:h-[40vw] max-w-[600px] max-h-[600px] bg-[radial-gradient(ellipse_at_center,rgba(244,63,94,0.04),transparent_60%)] blur-[100px]" />
          ...
          <motion.div variants={container} initial="hidden" animate="show" className="space-y-8 w-full">
            ...
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-6 md:gap-8 mb-4 md:mb-8">
              <motion.div variants={item}>
                ...
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter leading-none">Trade History.</h1>
              </motion.div>
              ...
              <motion.div variants={item} className="flex gap-4 w-full xl:w-auto overflow-x-auto no-scrollbar pb-2 snap-x">
                <div className="snap-start bg-white/60 dark:bg-[#0A0A0A]/80 backdrop-blur-xl border border-black/5 dark:border-white/[0.08] rounded-2xl p-4 md:p-5 min-w-[150px] md:min-w-[180px] shadow-sm relative overflow-hidden group">
                  ...
                  <p className="text-xl md:text-2xl font-mono font-black text-emerald-500">+$12,480.25</p>
                </div>
                <div className="snap-start bg-white/60 dark:bg-[#0A0A0A]/80 backdrop-blur-xl border border-black/5 dark:border-white/[0.08] rounded-2xl p-4 md:p-5 min-w-[140px] md:min-w-[160px] shadow-sm">
                  ...
                  <div className="flex items-end gap-2">
                    <p className="text-xl md:text-2xl font-mono font-black">68.5%</p>
                ...
                <div className="snap-start bg-white/60 dark:bg-[#0A0A0A]/80 backdrop-blur-xl border border-black/5 dark:border-white/[0.08] rounded-2xl p-4 md:p-5 min-w-[140px] md:min-w-[160px] shadow-sm">
                  ...
                  <p className="text-xl md:text-2xl font-mono font-black text-zinc-400">-$412.50</p>
                </div>
              </motion.div>
            </div>
            ...
            <motion.div variants={item} className="bg-white/60 dark:bg-[#0A0A0A]/80 backdrop-blur-3xl border border-black/5 dark:border-white/[0.08] rounded-[24px] md:rounded-[40px] shadow-2xl overflow-hidden flex flex-col w-full min-w-0">
              ...
              <div className="p-4 md:p-8 border-b border-black/5 dark:border-white/[0.05] flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 md:gap-6 bg-black/[0.01] dark:bg-white/[0.01]">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3 md:gap-4 w-full lg:w-auto">
                  ...
                  <button onClick={() => exportCsv('tradex-trades.csv', searchedTrades)} className="w-full sm:w-auto justify-center px-5 py-2.5 rounded-xl border border-black/10 dark:border-white/10 text-sm font-bold hover:bg-black/5 dark:hover:bg-white/5 transition-colors flex items-center gap-2">
              ...
              <div className="overflow-x-auto w-full no-scrollbar p-0 sm:p-4 md:p-8">
                <table className="w-full text-left border-collapse min-w-[900px] md:min-w-[1100px]">
                  <thead>
                    <tr className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest border-b border-black/5 dark:border-white/[0.05]">
                      ...
                      <th className="py-4 md:pb-6 font-medium text-center w-[10%]">Action</th>
                      <th className="py-4 md:pb-6 font-medium text-right w-[15%]">Size</th>
                      <th className="py-4 md:pb-6 font-medium text-right w-[15%]">Exec Price</th>
                      <th className="py-4 md:pb-6 font-medium text-right w-[10%]">Fee</th>
                      <th className="py-4 md:pb-6 pr-4 md:pr-6 font-medium text-right w-[20%]">Realized PnL</th>
                    </tr>
                  </thead>
                    ...
                    <AnimatePresence mode="popLayout">
                      {searchedTrades.map((trade, index) => (
                        ...
                        >
                          ...
                          {/* Size */}
                          <td className="py-4 md:py-6 text-right font-mono font-medium text-sm text-zinc-600 dark:text-zinc-300">
                            {trade.size}
                          </td>
                          ...
                          {/* Fee */}
                          <td className="py-4 md:py-6 text-right font-mono text-xs text-zinc-500">
                            {trade.fee}
                          </td>

                          {/* Realized PnL */}
                          <td className="py-4 md:py-6 pr-4 md:pr-6 rounded-r-2xl text-right">
                            {trade.isWin === null ? (
                              <span className="font-mono text-zinc-500 font-medium">-</span>
                            ) : (
                              <div className="flex flex-col items-end">
                                <span className={`font-black font-mono text-base md:text-lg ${trade.isWin ? 'text-emerald-500' : 'text-rose-500'}`}>
                                  {trade.realizedPnL}
                                </span>
                ...
                {searchedTrades.length === 0 && (
                  <div className="py-12 md:py-20 text-center flex flex-col items-center justify-center px-4">
                    ...
                    <p className="text-base md:text-lg font-bold text-zinc-400">No trades match this filter.</p>
                  </div>
                )}
              </div>
              ...
              <div className="p-4 md:p-6 border-t border-black/5 dark:border-white/[0.05] flex justify-between items-center bg-black/[0.01] dark:bg-white/[0.01]">
                <p className="text-[10px] md:text-xs font-bold text-zinc-500 uppercase tracking-widest">Showing {searchedTrades.length} records</p>
