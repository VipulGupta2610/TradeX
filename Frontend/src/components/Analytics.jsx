...
export default function Analytics() {
  ...
  const kpiData = [
    { 
      ...
      value: `${metrics?.unrealizedPnl >= 0 ? '+' : '-'}₹${Math.abs(metrics?.unrealizedPnl || 0).toLocaleString('en-IN', { maximumFractionDigits: 2 })}`, 
      trend: `${pnlPercent.toFixed(2)}%`, 
      isPositive: (metrics?.unrealizedPnl || 0) >= 0 
    },
    { 
      label: 'Executed Orders', 
      value: String(completed.length), 
      trend: `${metrics?.openOrders || 0} open`, 
      isPositive: true 
    },
    { 
      label: 'Portfolio Equity', 
      value: `₹${(metrics?.totalEquity || 0).toLocaleString('en-IN', { maximumFractionDigits: 0 })}`, 
      trend: `${positions?.length || 0} assets`, 
      isPositive: true 
    },
    { 
      label: 'Capital at Risk', 
      value: `₹${(metrics?.invested || 0).toLocaleString('en-IN', { maximumFractionDigits: 0 })}`, 
      trend: `${metrics?.totalEquity ? (metrics.invested / metrics.totalEquity * 100).toFixed(1) : 0}%`, 
      isPositive: (metrics?.invested || 0) <= (metrics?.totalEquity || 0) 
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0A0A0A] text-slate-900 dark:text-slate-100 transition-colors duration-300 font-sans selection:bg-blue-500/20 relative overflow-hidden">
      ...
      {/* --- TOP NAVIGATION --- */}
      <nav className="relative z-50 px-6 md:px-10 py-5 flex justify-between items-center border-b border-slate-200 dark:border-white/10 bg-white/70 dark:bg-[#0A0A0A]/70 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-blue-600 text-white flex items-center justify-center rounded-lg font-bold text-sm shadow-sm">
            TX
          </div>
          <span className="font-semibold tracking-widest text-xs text-slate-600 dark:text-slate-400">ANALYTICS</span>
        </div>

        <div className="flex items-center gap-4">
          <button 
            onClick={() => setIsDark(!isDark)} 
            className="p-2.5 rounded-full text-slate-500 hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-white/10 transition-colors"
          >
            {isDark ? (
          ...
          <Link to="/TradingTerminal" className="px-5 py-2 rounded-lg bg-slate-900 dark:bg-white text-white dark:text-black font-medium text-sm hover:opacity-90 transition-opacity shadow-sm">
            Execution Terminal
          </Link>
        </div>
      </nav>
      ...
      {/* --- MAIN CONTENT AREA --- */}
      <main className="relative z-10 w-full max-w-[1400px] mx-auto px-6 md:px-10 py-10">
            ...
            <motion.div variants={item} className="lg:col-span-2 bg-white dark:bg-[#111] border border-slate-200 dark:border-white/10 rounded-2xl p-6 md:p-8 shadow-sm flex flex-col">
              ...
              <div className="flex-1 w-full min-h-[300px] relative mt-4">
                <svg className="absolute inset-0 w-full h-full overflow-visible" viewBox="0 0 1000 300" preserveAspectRatio="none">
                  ...
                  {/* Gentle Grid Lines */}
                  {[75, 150, 225].map(y => (
                    <line key={y} x1="0" y1={y} x2="1000" y2={y} stroke="currentColor" className="text-slate-100 dark:text-white/5" strokeWidth="1" />
                  ))}
