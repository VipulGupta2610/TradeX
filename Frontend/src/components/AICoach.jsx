...
export default function AICoach() {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] dark:bg-[#020202] text-black dark:text-white transition-colors duration-500 font-sans selection:bg-cyan-500/30 relative">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden fixed bg-[#020202]">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(transparent_50%,rgba(0,0,0,0.2)_50%)] bg-[size:100%_4px] mix-blend-overlay opacity-30" />
      ...
      <nav className="relative z-50 px-8 py-6 flex justify-between items-center border-b border-black/5 dark:border-white/5 backdrop-blur-xl bg-white/40 dark:bg-[#020202]/40">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold tracking-[0.2em] text-sm text-zinc-800 dark:text-zinc-300">AI CO-PILOT</span>
        </div>
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="space-y-8">
          ...
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-8 mb-4">
            <motion.div variants={item}>
              <h2 className="text-sm font-bold text-zinc-500 uppercase tracking-widest mb-2 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-cyan-400 shadow-[0_0_8px_rgba(34,211,238,0.8)] animate-pulse" /> Agent 04 Online
              </h2>
              <h1 className="text-5xl md:text-6xl font-black tracking-tighter leading-none bg-gradient-to-r from-cyan-400 to-emerald-400 bg-clip-text text-transparent">
                Trading Mentor.
              </h1>
            </motion.div>
            ...
            <motion.div variants={item} className="flex p-1.5 bg-black/5 dark:bg-white/5 rounded-3xl border border-black/5 dark:border-white/[0.05] backdrop-blur-md">
              <div className="px-6 py-3 flex items-center gap-4 border-r border-white/5">
                <div className="w-10 h-10 rounded-full bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center">
                  <svg className="w-5 h-5 text-cyan-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                </div>
                <div>
                  <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Model Engine</p>
                  <p className="font-mono font-bold text-sm">TRADEX-LLM-v2</p>
                </div>
              </div>
              <div className="px-6 py-3 flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center">
                  <svg className="w-5 h-5 text-emerald-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                </div>
                <div>
                  <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Analysis Feed</p>
                  <p className="font-mono font-bold text-sm text-emerald-500">Live Sync</p>
                </div>
              </div>
            </motion.div>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 h-[700px]">
            ...
            <motion.div variants={item} className="xl:col-span-8 bg-white/60 dark:bg-[#0A0A0A]/80 backdrop-blur-3xl border border-black/5 dark:border-white/[0.08] rounded-[40px] shadow-2xl flex flex-col overflow-hidden relative">
              ...
              {/* Chat Header */}
              <div className="px-8 py-6 border-b border-white/[0.05] bg-black/[0.02] flex justify-between items-center z-10">
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-cyan-400 to-emerald-400 p-[1px]">
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-6 scrollbar-hide z-10 flex flex-col">
                <AnimatePresence initial={false}>
                    ...
                    >
                      <div className={`max-w-[80%] flex flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                        <div className={`p-5 rounded-3xl backdrop-blur-md text-sm md:text-base leading-relaxed ${
                          msg.sender === 'user' 
                            ? 'bg-white/10 dark:bg-white/[0.05] border border-white/10 text-white rounded-tr-sm' 
                            : 'bg-black/5 dark:bg-[#111] border border-cyan-500/20 text-zinc-200 rounded-tl-sm shadow-[0_0_20px_rgba(34,211,238,0.05)]'
                        }`}>
                          {msg.text}
                          ...
                          {msg.hasAction && (
                            <div className="mt-4 pt-4 border-t border-white/10 flex gap-3">
                              <button className="px-4 py-2 rounded-xl bg-cyan-500/20 text-cyan-400 font-bold text-xs hover:bg-cyan-500/30 transition-colors border border-cyan-500/30">
                                Enable Cooldown
                              </button>
                              <button className="px-4 py-2 rounded-xl bg-white/5 text-white font-bold text-xs hover:bg-white/10 transition-colors border border-white/10">
                                View Analysis
                              </button>
                            </div>
                          )}
                        </div>
                        <span className="text-[10px] text-zinc-600 font-mono mt-2 px-2">{msg.time}</span>
                      </div>
                    </motion.div>
                  ))}
                  ...
                  {isTyping && (
                    ...
                    >
                      <div className="p-5 rounded-3xl bg-black/5 dark:bg-[#111] border border-cyan-500/20 rounded-tl-sm flex gap-1.5 items-center">
              ...
              {/* Chat Input */}
              <div className="p-6 border-t border-white/[0.05] bg-black/[0.02] z-10">
                <form onSubmit={handleSendMessage} className="relative flex items-center">
                  <input 
                    ...
                    className="w-full bg-white dark:bg-[#0A0A0A] border border-black/10 dark:border-white/10 rounded-2xl pl-6 pr-16 py-5 text-sm font-medium focus:outline-none focus:border-cyan-500/50 transition-colors shadow-inner"
                  />
                  <button 
                    type="submit"
                    disabled={!inputValue.trim() || isTyping}
                    className="absolute right-3 w-10 h-10 bg-cyan-500 rounded-xl flex items-center justify-center text-black hover:bg-cyan-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_0_15px_rgba(34,211,238,0.4)]"
                  >
            ...
            <motion.div variants={item} className="xl:col-span-4 flex flex-col gap-6">
              ...
              <div className="bg-white/60 dark:bg-[#0A0A0A]/80 backdrop-blur-3xl border border-black/5 dark:border-white/[0.08] rounded-[32px] p-8 shadow-xl flex-1 relative overflow-hidden group">
                ...
                <div className="flex justify-between items-center mb-8">
                  ...
                  <span className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-rose-500/10 text-rose-500 text-[10px] font-bold border border-rose-500/20">
                    ...
                    <div key={idx}>
                      <div className="flex justify-between items-end mb-3">
                        ...
                        <div className="flex items-baseline gap-1">
                          <span className={`text-2xl font-black font-mono ${metric.value < metric.target ? 'text-white' : 'text-emerald-500'}`}>{metric.value}</span>
                      ...
                      <div className="relative w-full h-2 bg-white/5 rounded-full overflow-hidden">
                        {/* Target Marker */}
                        <div className="absolute top-0 bottom-0 w-0.5 bg-white/20 z-10" style={{ left: `${metric.target}%` }} />
                        <motion.div 
                          initial={{ width: 0 }} 
              ...
              <div className="bg-white/60 dark:bg-[#0A0A0A]/80 backdrop-blur-3xl border border-black/5 dark:border-white/[0.08] rounded-[32px] p-8 shadow-xl">
                ...
                <div className="space-y-4">
                  <div className="p-4 rounded-2xl bg-black/20 dark:bg-[#050505] border border-white/5 hover:border-white/10 transition-colors cursor-pointer group">
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-6 h-6 rounded-full bg-blue-500/20 flex items-center justify-center">
                        <svg className="w-3 h-3 text-blue-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      </div>
                      <p className="text-xs font-bold text-white group-hover:text-blue-400 transition-colors">Reduce Position Sizing</p>
                    </div>
                    <p className="text-xs text-zinc-400 leading-relaxed pl-9">Volatility on Crypto assets is 3x standard deviation. Scale down by 50%.</p>
                  </div>

                  <div className="p-4 rounded-2xl bg-black/20 dark:bg-[#050505] border border-white/5 hover:border-white/10 transition-colors cursor-pointer group">
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-6 h-6 rounded-full bg-emerald-500/20 flex items-center justify-center">
                        <svg className="w-3 h-3 text-emerald-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      </div>
                      <p className="text-xs font-bold text-white group-hover:text-emerald-400 transition-colors">Setup Alignment High</p>
                    </div>
                    <p className="text-xs text-zinc-400 leading-relaxed pl-9">Market conditions heavily favor your "Breakout Pullback" edge today.</p>
                  </div>
                </div>
              </div>
